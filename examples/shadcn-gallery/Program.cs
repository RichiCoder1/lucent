...
internal static class Program
{
    ...
    private static void CaptureWindow(bool dark)
    {
        ...
        try
        {
            ...
            var utility = (Border)controls.Single(control => control.Name == "UtilityCatalogEvidence");
            var conflict = (Border)controls.Single(control => control.Name == "UtilityConflictEvidence");
            var typography = (TextBlock)controls.Single(control => control.Name == "UtilityTypographyEvidence");
            ...
            Require(controls.Single(control => control.Name == "UtilityForegroundEvidence") is Button { Foreground: not null },
                "The templated-control foreground utility did not resolve.");
            Require(controls.Single(control => control.Name == "UtilityOpacityEvidence").Opacity == 1 &&
                    !controls.Single(control => control.Name == "UtilityHiddenEvidence").IsVisible,
                "Opacity/visibility utilities did not resolve.");
            Require(controls.Single(control => control.Classes.Contains("hover:bg-primary")) is Button { Background: not null } &&
                    controls.Single(control => control.Classes.Contains("focus:border-ring")) is TextBox { BorderBrush: not null } &&
                    controls.Single(control => control.Classes.Contains("focus-visible:border-ring")) is Button { BorderBrush: not null } &&
                    controls.Single(control => control.Classes.Contains("disabled:opacity-50")).Opacity == .5 &&
                    controls.Single(control => control.Classes.Contains("checked:bg-primary")) is CheckBox { Background: not null } &&
                    controls.Single(control => control.Classes.Contains("selected:bg-muted")) is ListBoxItem { Background: not null },
