...
"@ | Set-Content $config
    ...
    $generatedText = ($generated | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
    ...
    Copy-Item (Join-Path (Split-Path $consumer) 'Consumer.csproj'), (Join-Path (Split-Path $consumer) 'Program.cs'), (Join-Path (Split-Path $consumer) 'TransitionConsumer.cs'), (Join-Path (Split-Path $consumer) 'Tokens.cs'), (Join-Path (Split-Path $consumer) 'Widget.lui'), (Join-Path (Split-Path $consumer) 'MotionWidget.lui'), (Join-Path (Split-Path $consumer) 'packages.lock.json') $matrix
    ...
    if ($before.Count -ne 2) { throw 'Baseline did not produce the checked-in generated documents.' }
    $beforeText = Get-Content $before[0].FullName -Raw
    Set-Content (Join-Path $matrix 'Added.lui') 'internal component Added() { <Row /> }'
    Invoke-Dotnet @('build', $matrixProject, '--no-restore', '-t:Rebuild', '-warnaserror')
    if ((Get-ChildItem (Join-Path $matrix 'obj') -Recurse -Filter 'Lucent.Lui.*.g.cs').Count -ne 3) { throw 'Adding a .lui did not add one generated source.' }
    ...
    Invoke-Dotnet @('build', $matrixProject, '--no-restore', '-t:Rebuild', '-warnaserror')
    if ((Get-ChildItem (Join-Path $matrix 'obj') -Recurse -Filter 'Lucent.Lui.*.g.cs').Count -ne 2) { throw 'Deleting a .lui left stale generated source.' }
    ...
    if ($renamed.Count -ne 2 -or (($renamed | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n") -notmatch 'Renamed\.lui') { throw 'Renaming a .lui left stale identity or did not emit the new identity.' }
    ...
    # The published consumer also exercises .lui menu binding, cursor styles, content forwarding, and retained payloads through the packed SDK.
    Copy-Item (Join-Path $root 'tests/Lucent.Lui.Sdk.Fixtures/ContextMenu/*.lui'), (Join-Path $root 'tests/Lucent.Lui.Sdk.Fixtures/ContextMenu/ContextMenuConsumer.cs') $matrix
