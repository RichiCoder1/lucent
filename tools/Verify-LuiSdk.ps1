...
"@ | Set-Content $config
    ...
    if (!$generated -or (($generated | Get-Content -Raw) -notmatch '<auto-generated/>' -or ($generated | Get-Content -Raw) -notmatch 'GeneratedCodeAttribute')) { throw 'Generated inspection output lacks required markers.' }
    ...
    Copy-Item (Join-Path (Split-Path $consumer) 'Consumer.csproj'), (Join-Path (Split-Path $consumer) 'Program.cs'), (Join-Path (Split-Path $consumer) 'Widget.lui'), (Join-Path (Split-Path $consumer) 'packages.lock.json') $matrix
...
"@ | Set-Content $publishConfig
    ...
    $names = Get-ChildItem $publish -Recurse -File | Select-Object -ExpandProperty Name
    foreach ($forbidden in @('Microsoft.CodeAnalysis', 'Lucent.Lui.Compiler', 'Lucent.Lui.Generator', 'Lucent.Lui.Sdk')) {
        if ($names -match [regex]::Escape($forbidden)) { throw "NativeAOT publish contains forbidden runtime asset $forbidden." }
    }
}
finally {
