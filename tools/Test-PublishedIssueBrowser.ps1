...
public static class PublishedWindow {
...
function Assert-NativePreflight([string] $Asset) {
    ...
    try {
        ...
        $expected = "Lucent startup failed: Required native asset missing: $Asset."
...
function Stop-LaunchedProcess([Diagnostics.Process] $Process) {
    ...
    try {
        ...
        if (-not $Process.HasExited) {
            if ($Process.MainWindowHandle -ne 0) { [void][PublishedWindow]::PostMessage($Process.MainWindowHandle, 0x0010, [UIntPtr]::Zero, [IntPtr]::Zero) }
...
function Get-WindowPixel([IntPtr] $Hwnd, [int] $X, [int] $Y) {
    ...
    if (-not [PublishedWindow]::ClientToScreen($Hwnd, [ref]$origin)) { return $null }
    $desktop = [PublishedWindow]::GetDC([IntPtr]::Zero)
    if ($desktop -eq [IntPtr]::Zero) { return $null }
    try { return [PublishedWindow]::GetPixel($desktop, $origin.X + $X, $origin.Y + $Y) -band 0xffffff }
    finally { [void][PublishedWindow]::ReleaseDC([IntPtr]::Zero, $desktop) }
}

function Get-EffectiveAppearance {
    $contrast = [PublishedWindow+HIGHCONTRAST]::new()
    $contrast.Size = [Runtime.InteropServices.Marshal]::SizeOf([type][PublishedWindow+HIGHCONTRAST])
    if (-not [PublishedWindow]::SystemParametersInfo(0x0042, $contrast.Size, [ref]$contrast, 0)) { throw "Could not read Windows high-contrast state (Win32=$([Runtime.InteropServices.Marshal]::GetLastWin32Error()))." }
...
function Assert-ResizePixels([IntPtr] $Hwnd, [uint32] $Dpi, [PublishedWindow+RECT] $OriginalClient, $Appearance, [int] $Iteration) {
    $window = [PublishedWindow+RECT]::new()
    if (-not [PublishedWindow]::GetWindowRect($Hwnd, [ref]$window)) { throw "Iteration $Iteration could not read the original window rectangle." }
    $originalWidth = $window.Right - $window.Left
    $originalHeight = $window.Bottom - $window.Top
    if (-not [PublishedWindow]::SetWindowPos($Hwnd, [IntPtr]::Zero, 0, 0, $originalWidth + 320, $originalHeight + 240, 0x0016)) {
    ...
    try {
        ...
        do {
            Start-Sleep -Milliseconds 100
            $expanded = [PublishedWindow+RECT]::new()
            if (-not [PublishedWindow]::GetClientRect($Hwnd, [ref]$expanded)) { continue }
        ...
        if (-not [PublishedWindow]::SetWindowPos($Hwnd, [IntPtr]::Zero, 0, 0, $originalWidth, $originalHeight, 0x0016)) {
            throw "Iteration $Iteration could not restore the published window size."
        }
        $deadline = [Environment]::TickCount64 + 5000
        do {
            Start-Sleep -Milliseconds 100
            $restored = [PublishedWindow+RECT]::new()
            if ([PublishedWindow]::GetClientRect($Hwnd, [ref]$restored) -and
...
function Assert-SettingsListener([IntPtr] $Hwnd, [uint32] $Dpi, [PublishedWindow+RECT] $Client, $Appearance, [int] $Iteration) {
    if (-not [PublishedWindow]::PostMessage($Hwnd, 0x031A, [UIntPtr]::Zero, [IntPtr]::Zero)) { throw "Iteration $Iteration could not post WM_THEMECHANGED to the host listener." }
    Start-Sleep -Milliseconds 250
    [void](Assert-ScenePixels $Hwnd $Dpi $Client $Appearance $Iteration)
    'WM_THEMECHANGED chained through host listener'
}

$copy = Join-Path ([IO.Path]::GetTempPath()) ("lucent-published-app-" + [Guid]::NewGuid())
try {
    ...
    for ($iteration = 1; $iteration -le $Iterations; $iteration++) {
        ...
        try {
            ...
            if (-not [PublishedWindow]::SetWindowPos($process.MainWindowHandle, [IntPtr](-1), 0, 0, 0, 0, 0x0013)) { throw "Iteration $iteration could not expose the presentation for external sampling." }
            Start-Sleep -Milliseconds 500
            $client = [PublishedWindow+RECT]::new()
            if (-not [PublishedWindow]::GetClientRect($process.MainWindowHandle, [ref]$client) -or $client.Right -le 0 -or $client.Bottom -le 0) { throw "Iteration $iteration did not expose a positive client backing size." }
            $dpi = [PublishedWindow]::GetDpiForWindow($process.MainWindowHandle)
            ...
            if (-not [PublishedWindow]::PostMessage($process.MainWindowHandle, 0x0010, [UIntPtr]::Zero, [IntPtr]::Zero)) { throw "Iteration $iteration ordinary WM_CLOSE request failed." }
