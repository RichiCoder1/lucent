...
The published Windows TestHost includes two focused fixtures for the new framework seams. The layout fixture resizes a responsive `.lui` Grid between wide and compact arrangements, checks constrained paragraph line geometry, and paints at synthetic 100%, 150%, and 200% viewport scales. The input fixture focuses its native viewport and receives physical wheel and Ctrl+N/F/S events through the Windows adapter. These are framework TestHost proofs, not Light Notes application workflows; Light Notes maintains separate temporary-SQLite and workspace tests plus an opt-in FlaUI capture/save/close/reopen check against its published NativeAOT app. Its desktop check requires a coordinated uninterrupted input window; background UIA checks do not establish that an unoccluded screenshot was captured.
