...
For native scheduled pixels, publish the Windows TestHost, set `LUCENT_DESKTOP_HOST` to its executable, and run the desktop test project with `--filter 'FullyQualifiedName~PublishedMotionTests'`. The fixture tests autonomous intermediate pixels, application reduced motion, minimize/restore and shutdown during an active track. They take desktop focus and belong in a coordinated input window. See [TRANSITIONS.md](TRANSITIONS.md) for the public contract and custom-host frame sequence.
...
Application artwork has separate portable ICO/rendition tests, generated SDK resource and invalidation cases in the `Assets` suite, and a Windows HWND pixel test through `SDL_SetWindowIcon`, `WM_GETICON` and `DrawIconEx`. This distinguishes valid ICO bytes from correct installed alpha channels. The [asset memory probe](ASSET-MEMORY-EVIDENCE.md) records fresh-process codec and cache observations when investigating image performance; its sampled peaks are measurements, not allocator guarantees.
The `Assets` suite also compares multiple-APP1 JPEG and PNG eXIf orientation metadata with the pinned Skia decoder and requires generated catalogs to reject SVG inputs outside the runtime Secure Static encoded-size and structural policy.
...
./tools/Test-Repository.ps1 -Suite Assets
...
For retained scene-projection measurements, run `dotnet run --project tests/Lucent.Performance.Verifier -c Release -- --scene-projection`. It reports projection-only timing and current-thread allocation for wide and deep synthetic trees plus unchanged, same-breakpoint, changed-breakpoint, scale, scroll, and input updates through the real Issue Browser recipe. The probe installs every accepted scene and releases the superseded snapshot. Results are diagnostic and do not define a frame budget. See [scene-projection evidence](SCENE-PROJECTION-EVIDENCE.md) for the September 2026 baseline, optimization result, and limitations.
