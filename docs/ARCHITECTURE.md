...
`tools/Verify-Affected.ps1` is an optional focused runner for changed Core, renderer, Issue Browser, and Windows-presentation source/test files. Shared, build/configuration, architecture, unknown, deleted, and renamed paths fail closed to `tools/Verify-M1.ps1`. After M7, use the [pre-release verification policy](agents/verification.md): targeted affected checks for routine issues, with M1 reserved for planned releases, substantial cross-cutting changes, or risks that focused checks cannot contain. The optional runner still conservatively falls back to M1; direct affected commands are appropriate when that fallback exceeds the change risk.
