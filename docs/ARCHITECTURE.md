...
Public `Composition.When`, `Switch` and `ForEach` carry a theme into nested recipe factories. Their explicit overload accepts a `ThemeContext`; the shorter overload uses the exact parent's presentation theme and fails before allocating a region when the parent is not presented. Advanced callers building only raw retained elements use `WhenStructure`, `SwitchStructure` and `ForEachStructure`, whose factory context intentionally has no theme. Recipe-owned `MountContext` regions inherit their mount environment automatically. Parent adapters preserve context providers and service borrowing when selecting a presentation theme; raw structure adapters omit the theme while retaining that ancestry.

The canonical C# authoring interface uses reusable `ComponentRecipe` values for exactly-one-root components, `ContentRecipe` for zero-or-more structural contributions, and immutable ordered `ComponentContent` for transactional default content. The framework allocates every recipe root through the same public `ComponentRecipe.Create` seam used by built-ins and external custom-control authors. `ComponentRecipe.Defer` adds synchronous per-mount setup before resolving the final recipe; it reuses one retained root and the existing transaction/cleanup boundary. Stateful `.lui` lowers through this operation rather than adding a rerender engine or visual wrapper; see [ADR 0005](adr/0005-component-local-state.md). These capabilities are not component instances, virtual nodes, serializable templates, or rerender objects. General control templates and named `.lui` slots remain deferred until a real compositional control proves them.
...
The session keeps asynchronous startup, close preparation, service stop and cleanup on the desktop event loop. A preparation that publishes recoverable application state and returns false leaves the window and composition alive; repeated requests coalesce. An escaping exception or accepted close makes stop and cleanup terminal. Fatal abort cooperatively cancels in-flight preparation and starts cleanup without waiting for it. Core disposes the composition before lifecycle resources, and failures remain observable across independently attempted cleanup stages. Accepted saves must be drained by the application service before acceptance or accounted for by terminal service shutdown; they are separate from cancellation of obsolete scope-owned reads.
...
[ADR 0006](adr/0006-style-driven-layout.md) extends that surface with a retained generic `Layout`, style-selected built-in or custom nonvirtualizing algorithms, and typed child contributions. Custom algorithms receive a bounded measurement/placement context; composition, projection, ownership and virtualized realization remain framework responsibilities. Parameterized `.lui` styles and reactive `when (expression)` groups select arrangement and participation without replacing the content tree. Named window breakpoints are assigned from the logical viewport before layout; container queries are deferred. Application route intent and interaction state remain independent of those presentation rules.
...
`SemanticDeclaration.Create(role, name)` constructs one immutable payload through a validated builder. Common metadata is separate from a fixed aggregate of typed capabilities for editing, toggle, expansion, range, selection, collection membership and grid information. Capabilities describe data and supported operations; they contain no callbacks or platform providers. The behavior still owns command handlers, and declaring operations never grants action ownership. Pattern availability and command eligibility remain distinct: for example, a read-only ComboBox exposes a value pattern without permitting SetValue, while formatted status text does not acquire an editing pattern.

Snapshots share that declaration payload and add retained identity, reconciled enabled/focused/selected state and immutable children. Author name/description overlays share capabilities; supplemental tooltip descriptions are appended during projection. Clearing an author override restores behavior metadata. Visible editor draft text and caret ranges remain distinct from authoritative applied selection/toggle/range state. Confidential editing takes no text or value content and rejects conflicting registrations. Lazy input-availability and focus reconciliation still invalidate stale command generations; this migration does not add equality suppression. See the [semantic capability design and measurements](plans/semantic-capabilities.md).
