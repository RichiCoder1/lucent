...
One deterministic formatter owns whole-document and range formatting plus CLI/check surfaces. It formats `.lui` structure and embedded C# while preserving comments, literal contents and runtime-significant text. Builds never rewrite source. The [source-style guide](LUI-SOURCE-STYLE.md) defines the layout policy; [formatting and linting](LUI-FORMATTING.md) documents configuration, explicit safe fixes and failure behavior. Parser/binder errors retain their existing identities; authoring lints additionally cover proven default-content placement and optionally configured declaration order.
