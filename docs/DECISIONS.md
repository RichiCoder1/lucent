...
| Package metadata | Embed one internal versioned deterministic UTF-8 manifest before normal `CoreCompile`/signing, retain its isolated intermediate artifact, verify PE identity/resource afterward, and atomically promote only the last-successful fingerprint. Read references through non-executing `PEReader`/`MetadataReader` and a bounded project-generation cache; no source embedding, network access, runtime registry, assembly activation, or public stability promise. |
| Style metadata | Use one immutable producer-neutral `StyleClassCatalog` entry contract and manifest schema for later theme, global-style, and utility producers. Public generated catalog types are installable only when their owning style plan proves them; metadata does not install styles or define precedence. |
| Native templates | Support the bounded public `TemplateContent`/`IDeferredContent` form in the [native compatibility matrix](NATIVE_COMPATIBILITY.md); use explicit Avalonia C# for richer templates rather than claiming XAML parity. |
...
| Derived computation surface | `Computed<T>` accepts a cancellation-token `Task<T>` factory plus a required initial value; `.Value` stays stale while `.IsPending` is true and `.ErrorMessage` reports the latest failure | Keep the status facets while explicit `try`/`loading`/`catch` boundaries supply structural presentation; do not infer pending reads or coordinate reveal. |
| Reactive dependency binding | Bind compiler-recognized `State<T>` and `Computed<T>` reads by Roslyn symbol, deduplicate dependencies in declaration order, and treat reads hidden in opaque method bodies as snapshots. | Expand the recognized surface only when another reactive primitive has executable dependency, shadowing, and invalidation coverage. |
| Async failures | Use one owner-bound `OwnedComputed<T>` and the bounded `try (computed) { ... } loading { ... } catch (Exception error) { ... }` UI form. Preserve stale values during refresh; route unhandled failures to the root reporter. | Keep one explicit source and fixed branch precedence; defer automatic boundary discovery and reveal coordination. |
| Conditional regions | Support the narrow one-root conditional subset on a dedicated native content host, with owned branch lifetime and dependency-scoped updates. | Plan 003 may add nested, mixed, multi-root, and keyed-in-branch forms without replacing the ownership mechanism. |
