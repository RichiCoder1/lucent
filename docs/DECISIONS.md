...
| Derived computation surface | `Computed<T>` accepts a cancellation-token `Task<T>` factory plus a required initial value; `.Value` stays stale while `.IsPending` is true and `.ErrorMessage` reports the latest failure | Keep the status facets while the Plan 002 conditional subset supplies structural loading/error presentation; revisit the API after broader boundaries exist. |
...
| Async failures | Use one owner-bound `OwnedComputed<T>` and the bounded `try (computed) { ... } catch (Exception error) { ... }` UI form. Preserve stale values during refresh; route unhandled failures to the root reporter. | Add broader async syntax only after a concrete Workbench flow requires it. |
