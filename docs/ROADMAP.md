...
- Text, panel/layout primitives, button, single-line text field, multiline text area, selectable/list row, scroll viewport, virtualized list, loading/progress, simple error state, stock text/density roles, an optional minimal presentation base, accessible resizable split panes, and context menus with nested command groups and separators.
...
The independent September 8 review is being delivered through [#119](https://github.com/RichiCoder1/lucent/issues/119). The first integration is published as `0.3.0-dev.41.1` and consumed by Light Notes: it includes correctness fixes for reactive ownership, compiler binding, text rendering, input and popup placement, together with measured reactive, paragraph-cache and accessibility-navigation improvements. CI covers managed and NativeAOT contracts plus package-only consumption. Remaining work completes editor navigation, incremental tooling, parameterized-style refinements and the component organization trial in #143. Issue comments record source-bound checks and remaining platform limitations; this is an integration checkpoint, not closure of the parent.
...
The [component organization and framework-authored `.lui` plan](plans/core-component-organization.md), tracked in [#143](https://github.com/RichiCoder1/lucent/issues/143), first groups stock recipe, presentation and state code by component without changing the public API. It then proves clean same-assembly `.lui` generation and ships one substantive stock error/retry composite used by Issue Browser. C# retains runtime primitives and platform behavior. A separate controls package and broad conversion remain deferred until this trial establishes a useful boundary. Coordinate file moves with active correctness owners under #119.

## Queued images, icons and packaged assets

After the current #119/#143 work, [#144](https://github.com/RichiCoder1/lucent/issues/144) owns the accepted image and icon design. Its ordered children cover typed packaged assets (#145), owned asynchronous Image/Icon loading (#146), a verified secure static SVG adapter (#147), Lucide and accessible stock icon controls (#148), Windows and executable icons (#149), tooling and package documentation (#150), and Issue Browser adoption (#151). [Light Notes #4](https://github.com/RichiCoder1/light-notes/issues/4) is the independent proving consumer. The [accepted design](https://github.com/RichiCoder1/lucent/issues/144#issuecomment-5592602334) defines the contract and dependencies. These capabilities are queued, not part of the currently supported boundary.
