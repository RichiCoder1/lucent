...
- SDK-provided ordinary C# global/static usings make the built-in `Components` module and style fluency implicit, with an opt-out. Application and third-party component modules may add their own ordinary static imports. The compiler does not maintain aliases or a hidden symbol registry.
