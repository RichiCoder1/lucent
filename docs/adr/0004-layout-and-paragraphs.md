...
5. Assign final boxes and nested viewport rectangles. Realize fixed-height virtualized rows from that assigned rectangle, flush their retained effects, and perform one bounded final placement pass.
...
- A resize or responsive transition performs no more than two layout passes and one virtualization realization/flush pass. Unchanged paragraphs are cache hits.
...
- Paragraph caches are bounded by measured byte cost, initially 16 MiB per composition with least-recently-used eviction; eviction releases renderer-owned payloads on the composition owner.
