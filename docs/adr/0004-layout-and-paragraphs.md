...
A repeated state or a second unresolved geometry correction fails with a diagnostic; the engine must not iterate geometry until convergence. Breakpoint selection occurs outside child measurement, preventing a paragraph from changing the constraint that selected its responsive branch.

September 8, 2026 amendment for #106: responsive branches may introduce or remove nested responsive controls, such as a split pane. The initial design rejected any such structural change, which prevented composing these controls inside an adaptive workspace. A separate discovery phase assigns constraints to the newly established responsive tree, with at most eight discovery rounds; an unbounded recursive branch fails explicitly. This permits finite structural composition without relaxing the final assigned-geometry equality check or the virtualization realization boundary. Stable trees retain their existing pass count.

A virtualized viewport is an intrinsic measurement boundary: its source's scroll extent does not become an ancestor's desired size. Its assigned viewport comes from explicit dimensions or the surrounding Grid/Flex allocation, while the virtual region retains the full source extent for scrolling. This keeps nested lists bounded without requiring an arbitrary author-supplied height.

September 8, 2026 amendment for #108: auto-height measurement carries a known inline constraint through nested Flex containers, applying width constraints and subtracting padding before measuring wrapped content. A wrapping Row uses the existing Flex arrangement algorithm to determine its line extents, then corrects auto-height children at their assigned widths once before returning its height contribution. The existing Grid/Flex correction phase consumes container contributions as well as paragraph leaves. Per-projection intrinsic results are cached by element and effective inline constraint; paragraph cache reuse, responsive discovery limits and virtualized viewport boundaries remain intact. No convergence loop or additional global projection phase is introduced. Explicit height remains authoritative, and an unbounded inline constraint retains an unwrapped intrinsic contribution.

An unstyled conditional region retains lifetime and identity while passing its active child's parent-facing sizing and Grid placement through to layout. It does not introduce padding or paint, or change the child's internal arrangement. Explicitly presenting `ConditionalRegion.Region` opts into an ordinary authored container. This lets `.lui` branches contain growing workspaces without introducing an inaccessible implicit size constraint.
...
- Ordinary layout and virtualization perform no more than two layout passes and one realization/flush pass. A responsive transition with a stable set of responsive containers performs at most three layout passes: initial constraint assignment, post-branch viewport assignment, and final publication, with one realization/flush between the last two. Introducing nested responsive controls adds only the bounded structural-discovery rounds described above. Unchanged paragraphs are cache hits within each projection pass.
- After a single edit, cached unchanged paragraphs reuse their shaping and line breaking; source ranges and following line positions are rebased for the new document. A cold or evicted paragraph is shaped again. Caret moves and selection-only changes do not reshape text.
...
- Each renderer bounds complete shaped results (12 MiB), reusable paragraph drafts (4 MiB), and native text blobs (16 MiB), for 32 MiB combined estimated retained payload with least-recently-used eviction. Both shape caches also cap entries at 256. Paragraph keys include the complete shaping request and hard-break state; drafts contain managed font descriptors and relative UTF-16 glyph ranges. Shape estimates reserve the lazy grapheme/caret index footprint. Eviction releases associated renderer-owned blobs; an older immutable scene safely recreates those blobs on its next paint. Disposal releases all three caches.
