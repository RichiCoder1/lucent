...
The renderer characterization passes with 1, 100 and 1,000 active tracks, and Issue Browser's real virtualized hover/selection contract passes. The committed `bab7cc3` NativeAOT Windows TestHost passes 5/5 physical checks, including all three motion cases: autonomous intermediate pixels and active close, minimize/restore snapping, and application reduced-motion suppression. The other two cover pointer text selection and popup focus/lifetime. The [public guide](TRANSITIONS.md) records frame measurements. Light Notes commit `9c00f75` independently consumes the published `0.3.0-dev.55.1` packages, passes 42 app and 22 storage tests (one optional probe skipped), and passes published focus continuity and autosave/reopen checks 2/2.
