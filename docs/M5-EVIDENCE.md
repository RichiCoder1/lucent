...
Rerun the reference walkthrough (including the new header and inspector actions), keyboard walkthrough, UIA external proof and Accessibility Insights manual review, high-contrast/light/dark focus-pixel smoke, 10,000-row realization/performance and memory checks, managed and NativeAOT warning-clean contract/publish/smoke checks, clean-directory package/asset/notice/checksum inventory, and final visual review. Narrator remains supplemental. The M6 IME check is not behaviorally changed but remains part of the full post-M5 viability rerun.
