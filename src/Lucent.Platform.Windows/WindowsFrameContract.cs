    ...
    public WindowsViewport(int backingWidth, int backingHeight, float scale)
    {
        if (backingWidth < 0 || backingHeight < 0 || !float.IsFinite(scale) || scale <= 0)
            throw new ArgumentOutOfRangeException(
                nameof(scale),
                "Backing dimensions must be non-negative and scale must be finite and positive."
            );
...
internal enum CpuColorFormat
{
    Rgba8888,
    Bgra8888,
}

internal enum CpuAlphaFormat
{
    Premultiplied,
    Opaque,
}
...
internal static class WindowsPresentationContract
{
    public static readonly CpuSurfaceFormat SurfaceFormat = new(
        CpuColorFormat.Rgba8888,
        CpuAlphaFormat.Premultiplied
    );
    public const int VsyncInterval = 1;
}

internal readonly record struct CpuResourceDescriptor(
    int Width,
    int Height,
    CpuSurfaceFormat Format
)
{
    public CpuResourceDescriptor(WindowsViewport viewport)
        : this(viewport.BackingWidth, viewport.BackingHeight, viewport.Format) { }
}
...
internal sealed class CpuResourceState
{
    ...
    public void Commit(CpuResourceDescriptor descriptor)
    {
        _current = descriptor;
        CreationCount++;
    }
...
internal sealed class WindowsFrameScheduler
{
    ...
    public void Observe(WindowsFrameEvent @event, long? timestamp = null)
    {
        switch (@event)
        {
            case WindowsFrameEvent.Closed:
                IsOpen = false;
                _requested = false;
                _awaitingRenderable = false;
                _pendingRequest = null;
                break;
            case WindowsFrameEvent.Minimized:
                _minimized = true;
                _requested = false;
                _awaitingRenderable = false;
                _pendingRequest = null;
                break;
            case WindowsFrameEvent.Restored:
                _minimized = false;
                _requested = true;
                _awaitingRenderable = false;
                RecordRequest(FrameOperation.Other, timestamp);
                break;
            case WindowsFrameEvent.Moved:
                break;
            default:
                if (!_minimized)
                {
                    ...
                    _awaitingRenderable = false;
                    RecordRequest(
                        @event is WindowsFrameEvent.Resized or WindowsFrameEvent.PixelSizeChanged
                            ? FrameOperation.Resize
                            : FrameOperation.Other,
                        timestamp
                    );
                }
                break;
    ...
    {
        if (!_requested || _minimized)
            return false;
        if (!viewport.IsRenderable)
        {
            _awaitingRenderable = true;
            return false;
        }
        _requested = false;
        _awaitingRenderable = false;
    ...
    public void Complete(FrameTiming timing)
    {
        LastTiming = timing;
        PresentedFrames++;
    }
    ...
    private void RecordRequest(FrameOperation operation, long? timestamp)
    {
        ...
        if (pending.Operation == request.Operation)
            return;
        // Expose is a repaint notification, not a second cause for an input/resize operation.
        if (
            pending.Operation == FrameOperation.Other
            && request.Operation is FrameOperation.Input or FrameOperation.Resize
        )
        {
            _pendingRequest = request;
            return;
        ...
        if (
            request.Operation == FrameOperation.Other
            && pending.Operation is FrameOperation.Input or FrameOperation.Resize
        )
            return;
...
internal readonly record struct FrameTiming(
    TimeSpan Projection,
    TimeSpan Raster,
    TimeSpan Upload,
    TimeSpan Present
...
{
    public static FrameTiming FromTimestamps(
        long started,
        long projected,
        long rasterized,
        long uploaded,
    ...
    )
    {
        if (
            started > projected
            || projected > rasterized
            || rasterized > uploaded
            || uploaded > presented
        )
            throw new ArgumentOutOfRangeException(
                nameof(presented),
                "Frame phase timestamps must be ordered."
            );
        return new(
            Stopwatch.GetElapsedTime(started, projected),
            Stopwatch.GetElapsedTime(projected, rasterized),
            Stopwatch.GetElapsedTime(rasterized, uploaded),
            Stopwatch.GetElapsedTime(uploaded, presented)
        );
    }
}

internal enum FrameOperation
{
    Startup,
    Input,
    Resize,
...
internal readonly record struct FrameRequest(
