...
internal sealed class WindowsUiaDispatcher : IDisposable
{
    ...
    internal void RecordProvider(int cache, int maxCache, int stale) => _diagnostics.RecordProvider(cache, maxCache, stale);
    ...
    internal bool TryInvoke<T>(string callback, Func<T> action, out T result)
    {
        ...
        if (Volatile.Read(ref _disposed) != 0) { result = default!; return false; }
        if (Environment.CurrentManagedThreadId == _ownerThread)
        {
            try { result = action(); return true; }
            catch { result = default!; return false; }
        }
        if (!_slots.Wait(0)) { result = default!; return false; }
        var request = new Request(Interlocked.Increment(ref _nextId), callback, Environment.CurrentManagedThreadId, () => action(), () => _slots.Release());
        if (!_requests.TryAdd(request.Id, request)) { request.Release(); result = default!; return false; }
        _diagnostics.Enqueue(request);
        _pending.Enqueue(request);
        var @event = new SDL.Event { Type = _eventType };
        if (!_push(ref @event)) { _requests.TryRemove(request.Id, out _); request.CancelQueued(); result = default!; return false; }
        if (!request.Wait(_timeout))
        {
            if (request.CancelQueued()) { _diagnostics.Timeout(request); result = default!; return false; }
            request.Wait(Timeout.InfiniteTimeSpan);
        }
        _requests.TryRemove(request.Id, out _);
        if (request.Error is not null) { result = default!; return false; }
    ...
    {
        if (Environment.CurrentManagedThreadId != _ownerThread) throw new InvalidOperationException("UIA dispatcher must run on the SDL owner thread.");
        var count = 0;
        while (_pending.TryDequeue(out var request))
        {
            if (!request.TryBegin()) { _requests.TryRemove(request.Id, out _); request.Release(); continue; }
            _diagnostics.Start(request);
            try { request.Complete(request.Action()); _diagnostics.End(request); }
            catch (Exception error) { request.Fail(error); }
            finally { request.Release(); _diagnostics.Complete(request); count++; }
        }
        return count;
    }
    ...
    public void Dispose()
    {
        ...
        foreach (var request in _requests.Values) request.CancelQueued();
        while (_pending.TryDequeue(out var request)) { _requests.TryRemove(request.Id, out _); request.Release(); }
        _requests.Clear();
        _diagnostics.Write(_ownerThread);
    }

    private sealed class Request(long id, string callback, int callerThread, Func<object?> action, Action release)
    {
        ...
        internal void Complete(object? result) { Result = result; Volatile.Write(ref _state, 2); _complete.Set(); }
        internal void Fail(Exception error) { Error = error; Volatile.Write(ref _state, 2); _complete.Set(); }
        internal bool CancelQueued()
        {
            if (Interlocked.CompareExchange(ref _state, 3, 0) != 0) return false;
            _complete.Set(); return true;
        }
        internal void Release() { if (Interlocked.Exchange(ref _released, 1) == 0) release(); }
    }
    ...
    private sealed class UiaDiagnostics
    {
        ...
        private readonly bool _enabled = Environment.GetEnvironmentVariable("LUCENT_UIA_DIAGNOSTICS") == "1";
        private readonly string?[] _events = new string?[Capacity];
        private long _sequence, _frames, _reads, _actions, _providers, _cache, _maxCache, _stale, _timeouts, _roots, _listenerFailures;
        private string _phase = "startup";

        internal void SetPhase(string phase) { if (_enabled) Volatile.Write(ref _phase, phase); }
        internal void RecordFrame() { if (_enabled) Interlocked.Increment(ref _frames); }
        internal void RecordRead(string callback) { if (_enabled) { Interlocked.Increment(ref _reads); Event("read " + callback); } }
        internal void RecordAction(string callback) { if (_enabled) { Interlocked.Increment(ref _actions); Event("action " + callback); } }
        internal void RecordProvider(int cache, int maxCache, int stale) { if (_enabled) { Interlocked.Exchange(ref _providers, cache); Interlocked.Exchange(ref _cache, cache); Interlocked.Exchange(ref _maxCache, maxCache); Interlocked.Exchange(ref _stale, stale); } }
        internal void RecordRootDelivery() { if (_enabled) Interlocked.Increment(ref _roots); }
        internal void RecordListenerFailure() { if (_enabled) Interlocked.Increment(ref _listenerFailures); }
        internal void Enqueue(Request request) => Event($"enqueue id={request.Id} callback={request.Callback} caller={request.CallerThread} owner-phase={Volatile.Read(ref _phase)}");
        internal void Start(Request request) => Event($"start id={request.Id} callback={request.Callback} caller={request.CallerThread}");
        internal void End(Request request) => Event($"end id={request.Id} callback={request.Callback}");
        internal void Complete(Request request) => Event($"complete id={request.Id} callback={request.Callback} completed={request.Completed}");
        internal void Timeout(Request request) { if (_enabled) Interlocked.Increment(ref _timeouts); Event($"timeout id={request.Id} callback={request.Callback}"); }
        internal void Write(int ownerThread)
        {
            if (!_enabled) return;
            var last = Volatile.Read(ref _sequence); var first = Math.Max(1, last - Capacity + 1); var events = new List<string>();
            for (var sequence = first; sequence <= last; sequence++) if (Volatile.Read(ref _events[(int)(sequence % Capacity)]) is { } entry) events.Add(entry);
            Console.Error.WriteLine($"Lucent UIA diagnostics: owner={ownerThread} phase={Volatile.Read(ref _phase)} frame={Volatile.Read(ref _frames)} read={Volatile.Read(ref _reads)} action={Volatile.Read(ref _actions)} provider={Volatile.Read(ref _providers)} cache={Volatile.Read(ref _cache)} maxCache={Volatile.Read(ref _maxCache)} stale={Volatile.Read(ref _stale)} root={Volatile.Read(ref _roots)} listenerFailure={Volatile.Read(ref _listenerFailures)} timeout={Volatile.Read(ref _timeouts)} events=[{string.Join(";", events)}]");
        }
