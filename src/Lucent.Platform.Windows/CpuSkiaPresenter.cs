...
internal sealed class CpuSkiaPresenter : IDisposable
{
    ...
    {
        if (renderer == 0) throw new ArgumentOutOfRangeException(nameof(renderer));
        _renderer = renderer;
    }
    ...
    public PresenterPhaseTimestamps Present(RetainedScene scene, WindowsViewport viewport, SkiaSceneRenderer renderer)
    {
        ...
        if (!viewport.IsRenderable) throw new ArgumentOutOfRangeException(nameof(viewport));
        ...
        using var pixels = _surface.PeekPixels() ?? throw new InvalidOperationException("Skia surface did not expose CPU pixels.");
    ...
    {
        if (!_resources.NeedsRecreation(descriptor)) return;
    ...
    {
        if (_texture != 0) { SDL.DestroyTexture(_texture); _texture = 0; }
        _surface?.Dispose();
        _surface = null;
        _resources.Release();
    }
    ...
    private void CheckThread()
    {
        if (Environment.CurrentManagedThreadId != _ownerThread)
            throw new InvalidOperationException("CPU presenter access must remain on its owner thread.");
    }

    private void ThrowIfDisposed()
    {
        if (_disposed) throw new ObjectDisposedException(nameof(CpuSkiaPresenter));
    }
}

internal readonly record struct PresenterPhaseTimestamps(long Rasterized, long Uploaded, long Presented);
