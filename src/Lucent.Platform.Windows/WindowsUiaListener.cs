...
internal sealed unsafe partial class WindowsUiaListener : IDisposable
{
    ...
    internal WindowsUiaListener(nint window, WindowsUiaProvider provider)
    {
        ...
        _window = window;
        _handle = GCHandle.Alloc(new State(this, provider));
        if (!SetWindowSubclass(window, Procedure, _id, GCHandle.ToIntPtr(_handle)))
        {
            _handle.Free();
            throw new InvalidOperationException("SetWindowSubclass(UIA) failed.");
        }
    }
    ...
    public void Dispose()
    {
        if (Environment.CurrentManagedThreadId != _ownerThread)
            throw new InvalidOperationException(
                "UIA subclass disposal must remain on the SDL owner thread."
            );
        if (_disposed)
            return;
        _disposed = true;
        if (!_destroyed && !RemoveWindowSubclass(_window, Procedure, _id))
            throw new InvalidOperationException("RemoveWindowSubclass(UIA) failed.");
        Release();
    }
    ...
    )
    {
        ...
        try
        {
            ...
            if (message == WmGetObject && lParam == UiaRootObjectId)
            {
                if (state is null)
                    return 0;
                var delivery = WindowsUiaProvider.UiaReturnRawElementProvider(
                    window,
                    wParam,
                    lParam,
                    state.Provider.InterfacePointer
                );
                state.Listener.RootDeliveryCount++;
                state.Provider.RecordRootDelivery();
                return delivery;
            }
            var result = DefSubclassProc(window, message, wParam, lParam);
            if (message == WmNcDestroy && state is not null)
            {
                state.Listener._destroyed = true;
                state.Provider.MarkUnavailable();
        ...
        catch
        {
            try
            {
                state?.Provider.RecordListenerFailure();
            }
            catch { }
            return 0;
        }
