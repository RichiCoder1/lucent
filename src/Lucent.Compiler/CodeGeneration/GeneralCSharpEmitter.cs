...
{
    internal static string EmitGlobalStyles(string rootNamespace, BoundStyleSheet styles)
    {
        var writer = new CodeWriter();
        writer.Line("// <auto-generated />");
        writer.Line("using global::Avalonia.Styling;");
        writer.Line($"namespace {rootNamespace};");
        writer.Line();
        writer.Line("public sealed class LucentStyles : global::Avalonia.Styling.Styles");
        writer.Line("{"); writer.Indent();
        writer.Line("public LucentStyles()");
        writer.Line("{"); writer.Indent();
        foreach (var rule in styles.Rules)
        {
            ...
            {
                writer.Line($"Add(new global::Avalonia.Styling.Style(x => {BuildSelector(rule, target)})");
                writer.Line("{"); writer.Indent();
                writer.Line("Setters ="); writer.Line("{"); writer.Indent();
                foreach (var declaration in rule.Declarations.Where(declaration =>
                             declaration.PropertyName == "transition" || SupportsStyledProperty(target, declaration)))
                    writer.Line(declaration.PropertyName == "transition"
                        ? $"new global::Avalonia.Styling.Setter(global::Avalonia.Animation.Animatable.TransitionsProperty, {LowerTransitions(declaration.Value, target)}),"
                        : $"new global::Avalonia.Styling.Setter({PropertyField(target, declaration.PropertyName)}, {LowerStyleValue(declaration.PropertyName, declaration.Value)}),");
                writer.Unindent(); writer.Line("},"); writer.Unindent(); writer.Line("});");
            }
        }
        writer.Unindent(); writer.Line("}"); writer.Unindent(); writer.Line("}");
