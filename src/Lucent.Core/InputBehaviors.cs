...
public readonly record struct SemanticScrollState(
    ScrollOffset Offset,
        ...
        context.OnSemanticCommand(command =>
            command.Kind switch
            {
                SemanticCommandKind.Focus => context
                    .CompositionInput()
                    .FocusSemantic(context.Identity),
                SemanticCommandKind.Invoke => Invoke(),
                _ => false,
            }
        );
        int? armedPointer = null;
...
internal sealed class ScrollViewportBehavior(string name, ScrollViewportState state) : Behavior
{
    ...
    public override void Attach(BehaviorContext context)
    {
        ...
        {
            var handled =
                route.Command is { Kind: KeyCommandKind.Down, IsRepeat: false }
                && route.Command.Key switch
                {
                    Key.Left => route.ScrollBy(-40, 0),
                    Key.Right => route.ScrollBy(40, 0),
                    Key.Up => route.ScrollBy(0, -40),
                    Key.Down => route.ScrollBy(0, 40),
                    Key.Home => route.ScrollToStart(),
                    Key.End => route.ScrollToEnd(),
                    _ => false,
                };
