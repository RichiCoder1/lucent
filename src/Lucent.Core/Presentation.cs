...
public enum SemanticCommandKind { Focus, Invoke, SetValue, Select, Scroll }
public enum SemanticScrollEndpoint { None, Start, End }
public readonly record struct SemanticCommand(SemanticCommandKind Kind, string? Value = null, float Horizontal = 0, float Vertical = 0, SemanticScrollEndpoint Endpoint = SemanticScrollEndpoint.None)
{
    public void Validate()
    {
        if (!Enum.IsDefined(Kind) || !Enum.IsDefined(Endpoint) || !float.IsFinite(Horizontal) || !float.IsFinite(Vertical) ||
            (Kind != SemanticCommandKind.SetValue && Value is not null) ||
            (Kind != SemanticCommandKind.Scroll && (Horizontal != 0 || Vertical != 0 || Endpoint != SemanticScrollEndpoint.None)) ||
            (Kind == SemanticCommandKind.Scroll && Endpoint != SemanticScrollEndpoint.None && (Horizontal != 0 || Vertical != 0)))
            throw new ArgumentException("Semantic commands must be finite and match their declared operation.");
    }
}
public enum SemanticCommandResult { Applied, Rejected, Stale, Disabled }
...
{
    private readonly Composition _composition; private readonly ReactiveScope _scope; private readonly Dictionary<BehaviorState, bool> _state = []; private bool _attaching = true; private SemanticDeclaration? _semantic; private Func<SemanticCommand, bool>? _semanticCommand; private Action<bool>? _selectionChanged; private readonly Action _stateChanged;
    ...
    {
        CheckAttachment();
        if (_semantic is null || _semanticCommand is not null) throw new InvalidOperationException("Semantic commands require exactly one prior semantic declaration.");
    ...
    {
        CheckAttachment();
        if (_semantic is null || _selectionChanged is not null) throw new InvalidOperationException("Selection requires one semantic declaration and one writer.");
    ...
    internal SemanticDeclaration? Semantics => _semantic; internal Func<SemanticCommand, bool>? SemanticCommand => _semanticCommand; internal Action<bool>? SelectionChanged => _selectionChanged; internal IReadOnlyDictionary<BehaviorState, bool> State => _state; internal void Complete() => _attaching = false;
