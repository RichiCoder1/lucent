...
public sealed class SkiaSceneRenderer : ITextShaper, IDisposable
{
    ...
    public ShapedText Shape(TextMeasureRequest request)
    {
        ...
        if (request.Text.Length == 0)
            return Cache(request, new ShapedText(Identity(request, []), 0, 0, []));
        ...
        foreach (var piece in pieces)
        {
            ...
            if (MathF.Abs(width - result.Width) > .001f)
                throw new InvalidOperationException(
                    "SKShaper points and HarfBuzz advances disagree."
                );
            var metrics = font.Metrics;
            var ascent = metrics.Ascent;
            var descent = metrics.Descent;
            var family = face.FamilyName;
            ...
            pending.Add(
                new(
                    ...
                    collectionIndex,
                    family + "#" + collectionIndex.ToString(CultureInfo.InvariantCulture),
                    piece.Direction,
                    result.Width,
                    ascent,
                    descent,
        ...
        var lineAscent = pending.Min(run => run.Ascent);
        var lineDescent = pending.Max(run => run.Descent);
        var baseline = -lineAscent;
        var height = Checked(lineDescent - lineAscent);
        var origin = 0f;
    ...
    private void PaintRun(SKCanvas canvas, LayoutRect bounds, ShapedRun run, SKPaint brush)
    {
        ...
        try
        {
            ...
            canvas.DrawText(blob, bounds.X + run.OriginX, bounds.Y + run.Baseline, brush);
        }
        finally
        {
            canvas.Restore();
    ...
    private List<Piece> Itemize(TextMeasureRequest request)
    {
        ...
        var enumerator = StringInfo.GetTextElementEnumerator(request.Text);
    ...
    {
        if (_shapes.Count == ShapeCacheCapacity)
            _shapes.Remove(_shapeOrder.Dequeue());
        _shapes.Add(request, shaped);
        _shapeOrder.Enqueue(request);
        return shaped;
    }
