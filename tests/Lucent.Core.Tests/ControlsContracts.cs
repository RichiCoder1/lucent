...
internal static class ControlsContracts
{
    internal static int Run()
    {
        try
        {
            var first = Build(out var controls); using var composition = controls.Composition;
            Assert(first == Build(out var second), "Control composition dumps differ for equivalent recipes.");
    ...
    private static void ActivateAndSelect(Fixture fixture)
    {
        ...
        var buttonBox = scene.Boxes.Single(candidate => candidate.Identity.ElementId == fixture.Button.Id).Bounds;
        router.DispatchPointer(new(PointerCommandKind.Down, 0, buttonBox.X + 1, buttonBox.Y + 1, PointerButton.Secondary));
        ...
        Click(router, scene, fixture.Button, 1); Click(router, scene, fixture.Selectable, 2); Click(router, scene, fixture.Retry, 3);
        router.DispatchKey(new(KeyCommandKind.Down, Key.Enter));
        Assert(fixture.Calls.Button == 1 && fixture.Calls.Selection == 1 && fixture.Calls.Retry == 2, "Button/selectable/retry intents did not route through pointer and keyboard exactly once each.");
        var semantic = fixture.Composition.SemanticSnapshot()!;
        var selected = Flatten(semantic).Single(node => node.Name == "Selectable");
        Assert(selected.Selected && selected.Actions == SemanticAction.Select && Flatten(semantic).Single(node => node.Name == "Button").Actions == SemanticAction.Invoke, "Control semantics or selectable state diverged.");
        router.MoveFocus(FocusTraversalDirection.Next);
        Assert(fixture.Composition.Dump().Contains("FocusVisible", StringComparison.Ordinal), "Keyboard focus did not produce a reusable focus-visible variant.");
    }
    ...
    private static void StatusRetryThemesAndDisposal(Fixture fixture)
    {
        ...
        var stale = fixture.Composition.Input.DispatchPointer(new(PointerCommandKind.Move, 4, 1, 1));
        Assert(stale.Rejection == InputRejection.StaleScene, "Disposed control did not fail closed against its prior retained scene.");
        Expect<ArgumentOutOfRangeException>(() => Controls.Progress(fixture.Button, fixture.Theme, "bad", 2));
    }
    ...
    private static void ArmedPointerAndPaletteRegressions()
    {
        ...
        {
            var box = scene.Boxes.Single(candidate => candidate.Identity.ElementId == element.Id).Bounds;
            router.DispatchPointer(new(PointerCommandKind.Down, (int)element.Id, box.X + 1, box.Y + 1, PointerButton.Primary));
            router.DispatchPointer(new(PointerCommandKind.Up, (int)element.Id, box.X + box.Width + 1, box.Y + 1));
            router.DispatchPointer(new(PointerCommandKind.Down, (int)element.Id + 10, box.X + 1, box.Y + 1, PointerButton.Primary));
            router.DispatchPointer(new(PointerCommandKind.Cancel, (int)element.Id + 10, box.X + 1, box.Y + 1));
            Assert(calls == expectedCalls, "Captured outside/cancel gesture activated " + element.Name + ".");
            router.DispatchPointer(new(PointerCommandKind.Down, (int)element.Id + 15, box.X + 1, box.Y + 1, PointerButton.Primary));
            scene = SceneLayout.Project(composition, new(100, 60, 1), new EmptyShaper()); Assert(router.SetScene(scene), "Capture-loss replacement scene rejected.");
            router.DispatchPointer(new(PointerCommandKind.Up, (int)element.Id + 15, box.X + 1, box.Y + 1));
            expectedCalls++;
            Assert(calls == expectedCalls, "Reactive replacement cancelled a stable " + element.Name + " gesture.");
            router.DispatchPointer(new(PointerCommandKind.Down, (int)element.Id + 20, box.X + 1, box.Y + 1, PointerButton.Primary));
            router.DispatchPointer(new(PointerCommandKind.Up, (int)element.Id + 20, box.X + 1, box.Y + 1));
            expectedCalls++;
        }
    ...
    private static void CaptureContinuityAcrossReprojection()
    {
        ...
        RetainedScene Install()
        {
            ...
            {
                graph.Drain(); var scene = SceneLayout.Project(composition, new(100, 60, 1), new EmptyShaper());
                if (router.SetScene(scene)) return scene;
            }
            throw new InvalidOperationException("Capture continuity scene did not converge.");
        }
        (float X, float Y) Point(RetainedScene scene, Element element) { var box = scene.Boxes.Single(box => box.Identity.ElementId == element.Id).Bounds; return (box.X + 1, box.Y + 1); }
        void Down(RetainedScene scene, Element element, int pointer) { var point = Point(scene, element); router.DispatchPointer(new(PointerCommandKind.Down, pointer, point.X, point.Y, PointerButton.Primary)); }
        void Up(RetainedScene scene, Element element, int pointer) { var point = Point(scene, element); router.DispatchPointer(new(PointerCommandKind.Up, pointer, point.X, point.Y)); }

        var button = region.Items[0]; var selectable = region.Items[1];
        var scene = Install(); Down(scene, button, 40); refresh.Value = true; graph.Drain(); scene = Install(); Up(scene, button, 40);
        Assert(calls == 1 && !router.Dump().Contains("capture pointer=40", StringComparison.Ordinal), "Reactive replacement cancelled a structurally stable button gesture.");
        scene = Install(); Down(scene, selectable, 41); refresh.Value = false; graph.Drain(); scene = Install(); Up(scene, selectable, 41);
        Assert(calls == 2 && !router.Dump().Contains("capture pointer=41", StringComparison.Ordinal), "Reactive replacement cancelled a structurally stable selectable gesture.");

        scene = Install(); Down(scene, button, 42); rows.Value = [1, 0]; graph.Drain(); scene = Install(); Up(scene, button, 42);
        Assert(calls == 2 && router.Dump().Contains("captureLoss=SceneChanged", StringComparison.Ordinal), "Reordered retained path kept a captured button armed.");
        button = region.Items[1]; selectable = region.Items[0];
        scene = Install(); Down(scene, selectable, 43); rows.Value = [0]; graph.Drain(); scene = Install(); Up(scene, composition.Root, 43);
        Assert(calls == 2 && router.Dump().Contains("captureLoss=Disposed", StringComparison.Ordinal), "Removed selectable capture did not fail closed.");

        rows.Value = [0, 1]; graph.Drain(); button = region.Items[0]; selectable = region.Items[1]; scene = Install(); Down(scene, button, 44); theme.Theme = theme.Theme.Set(enabled, false); scene = Install(); Up(scene, button, 44);
        Assert(calls == 2 && router.Dump().Contains("captureLoss=Disabled", StringComparison.Ordinal), "Disabled button capture did not fail closed.");
        theme.Theme = theme.Theme.Set(enabled, true); scene = Install(); Down(scene, selectable, 45); theme.Theme = theme.Theme.Set(visible, false); scene = Install(); Up(scene, selectable, 45);
        Assert(calls == 2 && router.Dump().Contains("captureLoss=Hidden", StringComparison.Ordinal), "Hidden selectable capture did not fail closed.");
        theme.Theme = theme.Theme.Set(visible, true); scene = Install(); Down(scene, button, 46); _ = SceneLayout.Project(composition, new(100, 60, 1), new EmptyShaper()); Assert(!router.SetScene(scene), "Superseded capture scene was accepted.");
        Assert(calls == 2 && router.Dump().Contains("captureLoss=SceneChanged", StringComparison.Ordinal), "Superseded replacement kept capture armed.");
    }
    ...
    private static void MutableStateScrollAndRollback()
    {
        ...
        var conflict = composition.Child(composition.Root, "conflict"); conflict.AttachBehaviors(new ConflictBehavior()); var dump = composition.Dump(); var topology = graph.Dump();
        Expect<InvalidOperationException>(() => Controls.Loading(conflict, theme)); Expect<InvalidOperationException>(() => Controls.Selectable(conflict, theme, "Conflict"));
        Assert(composition.Dump() == dump && graph.Dump() == topology, "Control ownership conflict partially installed presentation/state.");
    }
    ...
    private static void PresentationPreflightAndControlAuthority()
    {
        ...
        var viewport = composition.Child(composition.Root, "viewport-conflict"); viewport.AttachBehaviors(new ConflictBehavior());
        var disposedTheme = new ThemeContext(composition.Root.Scope, ControlThemes.Light); disposedTheme.Dispose();
        ...
        })
        {
            ...
            graph.Drain();
            Assert(graph.Dump() == graphDump && composition.Dump() == compositionDump && SemanticSummary(composition.SemanticSnapshot()) == semantic && router.Dump() == input,
                "Failed control setup leaked graph, presentation, semantic, or input state.");
        }
        foreach (var attempt in new Action[]
        {
            () => Controls.Selectable(disposedSelectable, disposedTheme, "Selectable"), () => Controls.Loading(disposedLoading, disposedTheme), () => Controls.Progress(disposedProgress, disposedTheme, "Progress", .5f), () => Controls.Error(disposedError, disposedTheme, "Error"), () => Controls.ScrollViewport(disposedViewport, disposedTheme, "Viewport")
        })
        {
            ExpectPreflightFailure(attempt);
            graph.Drain();
    ...
    private static void ViewportFocusAndSceneInstallation()
    {
        ...
        graph.Drain(); Assert(router.SetScene(SceneLayout.Project(composition, new(100, 20, 1), new EmptyShaper())), "Clamped initial scroll scene rejected.");
        router.DispatchKey(new(KeyCommandKind.Down, Key.Tab));
        Assert(router.FocusedElement?.ElementId == viewport.Id && Flatten(composition.SemanticSnapshot()!).Single(node => node.Name == "Viewport") is { Focused: true, Actions: SemanticAction.Scroll }, "Viewport without focusable descendants was not reachable by Tab.");
        router.DispatchKey(new(KeyCommandKind.Down, Key.Home)); graph.Drain(); Assert(state.Offset == default, "Viewport Home did not scroll to the start.");
        Assert(router.SetScene(SceneLayout.Project(composition, new(100, 20, 1), new EmptyShaper())), "Home scene rejected.");
        router.DispatchKey(new(KeyCommandKind.Down, Key.Down)); graph.Drain(); Assert(state.Offset.Y == 40, "Viewport arrow key did not scroll deterministically.");
        Assert(router.SetScene(SceneLayout.Project(composition, new(100, 20, 1), new EmptyShaper())), "Arrow scene rejected.");
        router.DispatchKey(new(KeyCommandKind.Down, Key.End)); graph.Drain(); Assert(state.Offset.Y == 80, "Viewport End did not scroll to the bounded end.");
        theme.Theme = theme.Theme.Set(height, 20f); graph.Drain();
        Assert(!router.SetScene(SceneLayout.Project(composition, new(100, 20, 1), new EmptyShaper())) && state.Offset == default && router.DispatchKey(new(KeyCommandKind.Down, Key.Down)).Rejection == InputRejection.NoScene,
            "Content shrink accepted an immediately stale scroll scene.");
        graph.Drain(); Assert(router.SetScene(SceneLayout.Project(composition, new(100, 20, 1), new EmptyShaper())), "Shrunk-content reprojection was rejected.");
    ...
    {
        var graph = new ReactiveGraph(); using var composition = new Composition(graph, "installed-scroll");
        ...
        for (var key = 0; key < 3; key++) { _ = router.DispatchKey(new(KeyCommandKind.Down, Key.Right)); _ = router.DispatchKey(new(KeyCommandKind.Down, Key.Down)); }
        Assert(state.Offset == new ScrollOffset(80, 80), "Burst Right/Down exceeded the installed content extent before reprojection.");
        _ = router.DispatchKey(new(KeyCommandKind.Down, Key.Home)); Assert(state.Offset == default, "Home did not return both axes to start.");
        _ = router.DispatchKey(new(KeyCommandKind.Down, Key.End)); Assert(state.Offset == new ScrollOffset(80, 80), "End did not use the installed content extent.");
        theme.Theme = theme.Theme.Set(contentWidth, 20f).Set(contentHeight, 20f); graph.Drain();
        Assert(!router.SetScene(SceneLayout.Project(composition, new(20, 20, 1), new EmptyShaper())) && state.Offset == default, "Content shrink accepted an out-of-range installed offset.");
        graph.Drain(); Assert(router.SetScene(SceneLayout.Project(composition, new(20, 20, 1), new EmptyShaper())), "Shrunk-content reprojection was rejected.");
        theme.Theme = theme.Theme.Set(contentWidth, 100f).Set(contentHeight, 100f).Set(viewportWidth, 10f).Set(viewportHeight, 10f); graph.Drain();
        Assert(router.SetScene(SceneLayout.Project(composition, new(20, 20, 1), new EmptyShaper())) && router.MoveFocus(FocusTraversalDirection.Next), "Viewport change rejected a valid zero offset.");
        _ = router.DispatchKey(new(KeyCommandKind.Down, Key.End)); Assert(state.Offset == new ScrollOffset(90, 90), "Viewport change did not update the scroll extent.");
    }
    ...
    private static void VisualRejectionPreservesOnlyStableCapture()
    {
        ...
        RetainedScene Install()
        {
            for (var attempt = 0; attempt < 2; attempt++)
            {
                graph.Drain(); var scene = SceneLayout.Project(composition, new(100, 40, 1), new EmptyShaper());
        ...
        (float X, float Y) Point(RetainedScene scene) { var bounds = scene.Boxes.Single(box => box.Identity.ElementId == region.Items[0].Id).Bounds; return (bounds.X + 1, bounds.Y + 1); }
        var router = composition.Input; var scene = Install(); var point = Point(scene);
        _ = router.DispatchPointer(new(PointerCommandKind.Down, 70, point.X, point.Y, PointerButton.Primary));
        theme.Theme = theme.Theme.Set(otherEnabled, false); graph.Drain();
        Assert(!router.SetScene(SceneLayout.Project(composition, new(100, 40, 1), new EmptyShaper())) && router.DispatchPointer(new(PointerCommandKind.Move, 70, point.X, point.Y)).Rejection == InputRejection.NoScene,
            "Visual reconciliation left a rejected scene dispatchable.");
        scene = Install(); point = Point(scene); _ = router.DispatchPointer(new(PointerCommandKind.Up, 70, point.X, point.Y));
        Assert(activations == 1, "A visual-only rejection released a structurally stable capture.");

        theme.Theme = theme.Theme.Set(otherEnabled, true); scene = Install(); point = Point(scene);
        _ = router.DispatchPointer(new(PointerCommandKind.Down, 71, point.X, point.Y, PointerButton.Primary));
        theme.Theme = theme.Theme.Set(otherEnabled, false); graph.Drain(); Assert(!router.SetScene(SceneLayout.Project(composition, new(100, 40, 1), new EmptyShaper())), "Expected visual reconciliation rejection.");
        rows.Value = [1, 0]; graph.Drain(); _ = Install(); _ = router.DispatchPointer(new(PointerCommandKind.Up, 71, point.X, point.Y));
        Assert(activations == 1 && router.Dump().Contains("captureLoss=SceneChanged", StringComparison.Ordinal), "A structural replacement retained capture through visual reconciliation.");
    }

    private static void Click(InputRouter router, RetainedScene scene, Element element, int pointer)
    {
        var box = scene.Boxes.Single(candidate => candidate.Identity.ElementId == element.Id).Bounds;
        router.DispatchPointer(new(PointerCommandKind.Down, pointer, box.X + 1, box.Y + 1, PointerButton.Primary));
    ...
    private static string SemanticSummary(SemanticSnapshot? snapshot) => snapshot is null ? "-" : string.Join('|', Flatten(snapshot).Select(node => node.Identity.ElementId + ":" + node.Identity.Generation + ":" + node.Name + ":" + node.Actions));
    ...
    private sealed class Fixture(ReactiveGraph graph, Composition composition, ThemeContext theme, Element button, Element selectable, Element retry, Calls calls)
    {
        public ReactiveGraph Graph { get; } = graph; public Composition Composition { get; } = composition; public ThemeContext Theme { get; } = theme; public Element Button { get; } = button; public Element Selectable { get; } = selectable; public Element Retry { get; } = retry;
        public Calls Calls { get; } = calls;
    }

    private sealed class Calls { public int Button; public int Selection; public int Retry; }
    private sealed class ConflictBehavior : Behavior
    {
        public override string Name => "conflict"; public override BehaviorOwnership Ownership => BehaviorOwnership.Semantics;
        public override void Attach(BehaviorContext context) => context.SetSemantics(new(SemanticRole.Group, "conflict"));
    }
    ...
    private sealed class EmptyShaper : ITextShaper
    {
        public ShapedText Shape(TextMeasureRequest request)
        {
            ...
            var run = new ShapedRun("controls", "controls", 400, 5, 0, "controls", 0, "controls#0", request.Direction, request.Language, request.FontSize, 0, request.FontSize, -request.FontSize, 0, request.Text.Length, [glyph]);
