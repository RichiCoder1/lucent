...
Assert(
    root.Name.Text == "Components.Row"
        && root.Name.Span.Start == Complete.IndexOf("Components.Row", StringComparison.Ordinal),
    "qualified element name/span changed."
);
Assert(
    root.Attributes.Single(attribute => attribute.Name.Text == "Style").Value is LuiStyleWithSyntax,
    "colon inline style was not retained."
);
Assert(
    root.Attributes.Single(attribute => attribute.Name.Text == "P").Value is LuiExpressionSyntax,
...
Assert(
    conditional.Condition.Span.Start == Complete.IndexOf("values.Any", StringComparison.Ordinal),
...
Assert(
    missingElementClose.Component!.Body.OfType<LuiElementSyntax>().Single().CloseName.IsMissing
...
Assert(
    comments.Diagnostics.Count == 0
        && comments
            ...
            .SequenceEqual(new[] { "// header", "/* between */", "// component", "// style" })
...
Assert(
    commentRanged == LuiFormatter.FormatRange(commentRanged, commentRange, LuiLineEnding.Lf),
...
Assert(
    textRegions
        ...
        .SequenceEqual(new[] { "if only", "foreach item" }),
    "plain if/foreach text was parsed as a region."
);
var badRegion = LuiParser.Parse("internal component X() { if (ok { <A /> } <B /> }");
Assert(
    badRegion.Component!.Body.OfType<LuiElementSyntax>().Any(element => element.Name.Text == "B"),
...
Assert(
    ordered
        .Diagnostics.Select(diagnostic => diagnostic.Span.Start)
        .SequenceEqual(
            ordered.Diagnostics.Select(diagnostic => diagnostic.Span.Start).OrderBy(start => start)
        ),
...
Assert(
    !((LuiNamespaceSyntax)owned.TopLevel.OfType<LuiNamespaceSyntax>().Single()).Keyword.IsMissing
        && !((LuiNamespaceSyntax)owned.TopLevel.OfType<LuiNamespaceSyntax>().Single())
            .Semicolon
            .IsMissing
        && !((LuiUsingSyntax)owned.TopLevel.OfType<LuiUsingSyntax>().Single()).Semicolon.IsMissing
...
var missingExpression = (LuiExpressionSyntax)
    ((LuiElementSyntax)LuiParser.Parse("internal component X() { <A P={x").Component!.Body.Single())
        .Attributes.Single()
        .Value;
...
Assert(
    LuiParser.Parse("internal component X() { <A P={$\"{x, x + 1}\"} /> }").Diagnostics.Count == 0,
...
Assert(
    conditionalStyle.Diagnostics.Count == 0
        && conditionalStyle.Styles.Single().Assignments.Single().Expression.Text == "ok ? X : Y",
...
var variantMembers = (
    (LuiStyleWithSyntax)
        ((LuiElementSyntax)variantFirstDocument.Component!.Body.Single()).Attributes.Single().Value
).Members;
Assert(
...
Assert(
    rejectedParameters
        ...
        && rejectedParameters.Diagnostics.Count(diagnostic => diagnostic.Id == "LUI3004") == 4,
...
Assert(
    roslynNames.Diagnostics.Count == 0
        && ((LuiElementSyntax)roslynNames.Component!.Body.Single()).Name.Text == "global::Sample.A"
        && ((LuiElementSyntax)roslynNames.Component.Body.Single()).Attributes.Any(attribute =>
            attribute.Name.Text == "Sample.\\u00C5ngström"
        ),
...
{
    var source =
        "internal component X() { <A P={new[] { \"}\", " + i + " }.Length}> text " + i + "</A> }";
    var parsed = LuiParser.Parse(source);
    Assert(
        parsed.Diagnostics.Count == 0
            && LuiFormatter.Format(source) == LuiFormatter.Format(LuiFormatter.Format(source)),
...
Assert(
    lowered
        .Map.FromSource(new LuiSpan(loweredSource.IndexOf("Hello", StringComparison.Ordinal), 5))
        .Count > 0
        && lowered
            .Map.FromGenerated(
                lowered.Map.Entries.First(entry => entry.Kind == LuiMapKind.Expression).Generated
            )
            .Count > 0
...
var namedAttribute = ((LuiElementSyntax)loweredDocument.Component!.Body.Single()).Attributes.Single(
    attribute => attribute.Name.Text == "name"
);
...
var matrix = LuiCompiler.Compile(
    ...
    new LuiFreshnessIdentity("43", "matrix", new LuiDocumentIdentity("Matrix.lui"), "v2", "preview")
);
Assert(
    matrix.Success
...
var invalidAttributes = LuiCompiler.Compile(
    ...
    new LuiFreshnessIdentity("44", "bad", new LuiDocumentIdentity("Bad.lui"), "v1", "preview")
);
Assert(
...
var customCompilation = CSharpCompilation.Create(
    "custom",
    [CSharpSyntaxTree.ParseText(customApi, new CSharpParseOptions(LanguageVersion.Preview))],
    References()
);
var genericContent = LuiCompiler.Compile(
    ...
    customCompilation,
    new LuiFreshnessIdentity("45", "custom", new LuiDocumentIdentity("Custom.lui"), "v1", "preview")
);
Assert(
...
var genericEmpty = LuiCompiler.Compile(
    ...
    customCompilation,
    new LuiFreshnessIdentity("45", "custom", new LuiDocumentIdentity("Empty.lui"), "v1", "preview")
);
Assert(
...
var inline = LuiCompiler.Compile(
    ...
    new LuiFreshnessIdentity("46", "inline", new LuiDocumentIdentity("Inline.lui"), "v1", "preview")
);
Assert(
    inline.Success
...
var secondHover = duplicateVariantSource.IndexOf("Hover", firstHover + 1, StringComparison.Ordinal);
Assert(
...
)
{
    ...
    Assert(
        !prohibited.Success && prohibited.Diagnostics.Any(diagnostic => diagnostic.Id == "LUI2007"),
...
var guarded = LuiCompiler.Compile(
    ...
    new LuiFreshnessIdentity("47", "guards", new LuiDocumentIdentity("Guards.lui"), "v1", "preview")
);
Assert(
    guarded.Success
...
Assert(
    !lowered.Source!.Contains("/*lui*/", StringComparison.Ordinal)
        && lowered.Map.Entries.Where(entry => entry.Hidden).All(entry => entry.Source.Start < 0)
        && lowered
...
Assert(
    ...
    "comment map source failed to lower: "
        + string.Join(" | ", commentsResult.Diagnostics.Select(diagnostic => diagnostic.Message))
);
foreach (
    var comment in new[]
    {
