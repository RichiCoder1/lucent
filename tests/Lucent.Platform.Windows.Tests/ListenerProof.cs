...
internal static partial class ListenerProof
{
    ...
    internal static int Run()
    {
        if (!SDL.Init(SDL.InitFlags.Video))
            return Fail("SDL_Init: " + SDL.GetError());
        nint window = 0;
        WindowsSettingsListener? listener = null;
        try
        {
            window = SDL.CreateWindow(
                "Lucent Windows listener proof",
                160,
                120,
                SDL.WindowFlags.Resizable
            );
            if (window == 0)
                return Fail("SDL_CreateWindow: " + SDL.GetError());
            if (!SDL.ShowWindow(window))
                return Fail("SDL_ShowWindow: " + SDL.GetError());
            ...
            if (hwnd == 0)
                return Fail("SDL window did not expose an HWND.");
            ...
            {
                if (listener.TakePending())
                {
                    refreshed = true;
                    Paint(hwnd, Green);
                    Console.WriteLine("LISTENER-PROOF REFRESHED");
                }
                if (!refreshed)
                    Paint(hwnd, Red); // SDL may repaint after the initial marker; preserve the required pre-refresh state.
                if (
                    (SDL.EventType)@event.Type
                    is SDL.EventType.Quit
    ...
    {
        if (
            !GetClientRect(window, out var rectangle)
            || rectangle.Right <= 0
            || rectangle.Bottom <= 0
        )
        ...
        var dc = GetDC(window);
        if (dc == 0)
            throw new InvalidOperationException("GetDC(listener proof) failed.");
        var brush = CreateSolidBrush(color);
        if (brush == 0)
        {
            _ = ReleaseDC(window, dc);
            throw new InvalidOperationException("CreateSolidBrush(listener proof) failed.");
        }
        try
        {
            if (FillRect(dc, ref rectangle, brush) == 0)
                throw new InvalidOperationException("FillRect(listener proof) failed.");
        }
        finally
        {
    ...
    {
        Console.Error.WriteLine("Lucent Windows listener proof: " + message);
        return 1;
    }
