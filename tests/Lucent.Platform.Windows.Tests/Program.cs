...
if (args is ["--listener-proof"])
    return ListenerProof.Run();
if (args is ["--uia-fixture"])
    return UiaFixture.Run();
if (args is ["--virtualization-fixture"])
    return VirtualizationFixture.Run();
...
catch (Exception error)
{
    Console.Error.WriteLine(
        "Lucent.Platform.Windows presenter contract/AOT seam: FAIL: " + error.Message
    );
    return 1;
}
...
static void TelemetryContract()
{
    ...
    Assert(
        activityOperation == "input"
            ...
            && M6Diagnostics.AllowedTags.SequenceEqual([
...
static void UiaDispatcherContract()
{
    ...
    try
    {
        ...
        var worker = Task.Run(() =>
            dispatcher.TryInvoke("test", () => Environment.CurrentManagedThreadId, out var owner)
                ? owner
                : -1
        );
        ...
        if (
            ...
            throw new InvalidOperationException(
                "Failed SDL event pushes escaped the dispatcher's physical queue bound or did not release on dequeue."
            );
    }
    finally
    {
...
static void ReactiveWakeContract()
{
    ...
    try
    {
        ...
        using var fatalComposition = new Lucent.Core.Composition(fatalGraph, "reactive-wake-fatal");
        ...
        var raceValue = raceGraph.Async(_ => raceWork.Task, 0, "reactive-wake-dispose-race-value");
        _ = raceValue.Value;
