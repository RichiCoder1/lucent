...
internal static class VirtualizationFixture
{
    internal static int Run()
    {
        ...
        {
            Console.Error.WriteLine("Lucent virtualization fixture: " + error.Message);
            return 1;
        }
    }
    ...
    private static Composition Create(ReactiveGraph graph, out ThemeContext theme)
    {
        ...
        var search = composition.Child(composition.Root, "search");
        Controls.TextField(
            search,
            context,
            "Search",
            style: Style.Empty.Set(LayoutProperties.Width, 400f).Set(LayoutProperties.Height, 24f)
        );
        var values = composition.Root.Scope.Signal(
