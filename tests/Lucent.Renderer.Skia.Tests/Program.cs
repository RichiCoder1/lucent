...
{
    var identity = new ElementIdentity(1, 1); var blue = Brush.Solid(Color.Parse("#0000FF"));
    var overlap = new OpacitySceneNode(new(identity, SceneNodeKind.Opacity), new(0, 0, 8, 4), .5f,
    [new PaintSceneNode(new(identity, SceneNodeKind.Paint), new(0, 0, 6, 4), blue), new PaintSceneNode(new(identity, SceneNodeKind.Paint), new(2, 0, 6, 4), blue)]);
    using var pixels = new SKBitmap(8, 4, SKColorType.Rgba8888, SKAlphaType.Premul);
    using (var canvas = new SKCanvas(pixels)) { canvas.Clear(SKColors.Transparent); renderer.Render(new RetainedScene(1, new(8, 4, 1), [], [overlap], []), canvas); }
    Assert(pixels.GetPixel(1, 2).Alpha is >= 127 and <= 129 && pixels.GetPixel(3, 2).Alpha is >= 127 and <= 129, "Opacity was applied to each overlapping paint instead of once to the composited group.");

    var nested = new OpacitySceneNode(new(identity, SceneNodeKind.Opacity), new(0, 0, 4, 4), .5f,
    [new OpacitySceneNode(new(identity, SceneNodeKind.Opacity), new(0, 0, 4, 4), .5f, [new PaintSceneNode(new(identity, SceneNodeKind.Paint), new(0, 0, 4, 4), Brush.Solid(Color.Parse("#FF0000")))])]);
    using var nestedPixels = new SKBitmap(4, 4, SKColorType.Rgba8888, SKAlphaType.Premul);
    using (var canvas = new SKCanvas(nestedPixels)) { canvas.Clear(SKColors.Transparent); renderer.Render(new RetainedScene(1, new(4, 4, 1), [], [nested], []), canvas); }
    Assert(nestedPixels.GetPixel(2, 2).Alpha is >= 63 and <= 65, "Nested opacity did not multiply.");

    foreach (var scale in new[] { 1f, 1.25f, 1.5f, 2f })
    {
        var size = (int)(4 * scale); using var zeroPixels = new SKBitmap(size, size, SKColorType.Rgba8888, SKAlphaType.Premul);
        using var canvas = new SKCanvas(zeroPixels); canvas.Clear(SKColors.Transparent);
        renderer.Render(new RetainedScene(1, new(4, 4, scale), [], [new OpacitySceneNode(new(identity, SceneNodeKind.Opacity), new(0, 0, 4, 4), 0, [new PaintSceneNode(new(identity, SceneNodeKind.Paint), new(0, 0, 4, 4), blue)])], []), canvas);
        Assert(Enumerable.Range(0, size).SelectMany(y => Enumerable.Range(0, size).Select(x => zeroPixels.GetPixel(x, y).Alpha)).All(alpha => alpha == 0), "Opacity zero painted pixels at scale " + scale);
    }
    Assert(renderer.LiveTextBlobCount == 0, "Opacity groups retained renderer resources after a frame.");
}
static void AssertOpacityLayout(SkiaSceneRenderer renderer)
{
    foreach (var clip in new[] { false, true })
        foreach (var scale in new[] { 1f, 1.25f, 1.5f, 2f })
        {
            var graph = new ReactiveGraph(); using var composition = new Composition(graph, "opacity-layout"); var theme = new ThemeContext(composition.Root.Scope, new Theme("opacity-layout"));
            composition.Root.Present(theme, author: Style.Empty.Set(LayoutProperties.Axis, LayoutAxis.Column));
            var owner = composition.Child(composition.Root, "owner"); owner.Present(theme, author: Style.Empty.Set(LayoutProperties.Width, 4f).Set(LayoutProperties.Height, 4f).Set(LayoutProperties.Clip, clip).Set(VisualProperties.Opacity, .5f));
            var child = composition.Child(owner, "overflow"); child.Present(theme, author: Style.Empty.Set(LayoutProperties.Width, 8f).Set(LayoutProperties.Height, 4f).Set(VisualProperties.Background, Color.Parse("#0000FF")));
            var retained = SceneLayout.Project(composition, new(12, 8, scale), renderer); var group = retained.Nodes.OfType<OpacitySceneNode>().Single();
            var width = (int)MathF.Ceiling(12 * scale); var height = (int)MathF.Ceiling(8 * scale); using var pixels = new SKBitmap(width, height, SKColorType.Rgba8888, SKAlphaType.Premul);
            using (var canvas = new SKCanvas(pixels)) { canvas.Clear(SKColors.Transparent); renderer.Render(retained, canvas); }
            var inside = pixels.GetPixel((int)MathF.Floor(2 * scale), (int)MathF.Floor(2 * scale)); var overflow = pixels.GetPixel((int)MathF.Floor(6 * scale), (int)MathF.Floor(2 * scale));
            Assert(inside.Alpha is >= 127 and <= 129 && (clip ? overflow.Alpha == 0 && group.Bounds.Width == 4 : overflow.Alpha is >= 127 and <= 129 && group.Bounds.Width == 8),
                "SceneLayout opacity overflow/clip pixels or bounds changed at scale " + scale + " clip=" + clip);
        }
}
