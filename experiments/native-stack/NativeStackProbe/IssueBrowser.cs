...
internal static class IssueBrowser
{
    ...
    internal enum InputCommand { Search, OpenFilter, HighFilter, PageDown, End, Home, SelectFirst, Next, RemoveSelected, EditTitle, CommitTitle, RevertTitle, FailSearch, Retry, ToggleTheme, ReducedMotion, Tab, ShiftTab, EscapeToList }
        ...
        while (SDL.WaitEventTimeout(out var e, 20))
        {
            ...
            switch (type)
            {
                ...
                case SDL.EventType.MouseButtonDown:
                    if (e.Button.X < 96 && e.Button.Y is >= 180 and <= 224) state.Dispatch(InputCommand.OpenFilter);
                    else if (e.Button.X < 190 && e.Button.Y is >= 180 and <= 224) state.Dispatch(InputCommand.HighFilter);
    ...
    private sealed class BrowserRenderer(State state, bool? forcedDark = null) : ISkiaSceneRenderer
    {
        public void Render(RetainedScene scene, SKCanvas canvas)
        {
            ...
            Text("FILTERS", 20, 174, small, subtle); Rect(16, 188, 76, 30, accent, 15); Text("Open", 39, 208, body, dark ? "#18181b" : "#fafafa"); Rect(100, 188, 76, 30, raised, 15); Text("High", 124, 208, body, foreground);
    ...
    private sealed class State : IDisposable
    {
        ...
        private readonly ReactiveGraph _graph = new(); private readonly ReactiveSignal<string> _query; private readonly ReactiveSignal<string?> _status; private readonly ReactiveSignal<string?> _priority; private readonly ReactiveSignal<ThemeLayer> _theme;
        ...
        public State(IssueBrowserSeed seed)
        {
            ...
            _query = _graph.Signal("", "issues.query"); _status = _graph.Signal<string?>(null, "issues.status"); _priority = _graph.Signal<string?>(null, "issues.priority"); _theme = _graph.Signal(Themes.Light, "issues.theme");
            _filtered = _graph.Computed(() => _all.Where(issue => issue.Title.Contains(_query.Value, StringComparison.OrdinalIgnoreCase) && (_status.Value is null || issue.Status == _status.Value) && (_priority.Value is null || issue.Priority == _priority.Value)).ToArray(), "issues.filtered");
            _async = _graph.AsyncComputed(async cancellation => { var query = _query.Value; var status = _status.Value; var priority = _priority.Value; await Task.Delay(seed.AsyncDelayMilliseconds, cancellation); if (query == seed.FailureQuery) throw new InvalidOperationException("Search failed"); return _all.Where(issue => issue.Title.Contains(query, StringComparison.OrdinalIgnoreCase) && (status is null || issue.Status == status) && (priority is null || issue.Priority == priority)).ToArray(); }, "issues.search");
            ...
            _statusSemantic = new(new("issue-browser.status"), new(16, 188, 76, 30), new("#18181b", "#fafafa", 15), new("checkbox", "Open", null, ["toggle"]));
            _prioritySemantic = new(new("issue-browser.priority"), new(100, 188, 76, 30), new("#f4f4f5", "#09090b", 15), new("checkbox", "High", null, ["toggle"])); _root.Children.AddRange([_searchSemantic, _statusSemantic, _prioritySemantic]);
        ...
        public void Dispatch(InputCommand command, string? text = null)
        {
            switch (command)
            {
                case InputCommand.Search: SetQuery(text ?? ""); break; case InputCommand.OpenFilter: _status.Value = _status.Value is null ? "Open" : null; break; case InputCommand.HighFilter: _priority.Value = _priority.Value is null ? "High" : null; break;
        ...
        {
            _searchSemantic.Semantics = _searchSemantic.Semantics with { Value = Query }; _statusSemantic.Semantics = _statusSemantic.Semantics with { Value = _status.Value }; _prioritySemantic.Semantics = _prioritySemantic.Semantics with { Value = _priority.Value };
            _list.Viewport.Element.Semantics = _list.Viewport.Element.Semantics with { Value = Selected, Actions = ["select", "scroll"] }; foreach (var element in new[] { _searchSemantic, _statusSemantic, _prioritySemantic, _list.Viewport.Element }) element.Mark(DirtyFacet.Semantics); _projection.Project(Flatten(_root));
            return _snapshots.Semantics.Where(pair => pair.Key.Value is "issue-browser.search" or "issue-browser.status" or "issue-browser.priority" or "issues.viewport" or "issue-browser.title" or "issue-browser.retry").Select(pair => new IssueBrowserSemantic(pair.Key.Value switch { "issue-browser.search" => "search", "issue-browser.status" => "status", "issue-browser.priority" => "priority", "issues.viewport" => "issues", "issue-browser.title" => "title", _ => "retry" }, pair.Value.Role, pair.Value.Name, pair.Value.Value, pair.Value.Actions ?? [])).ToArray();
        }
        public IssueBrowserIdentity Identity() => new(Rows.Count, Realized, Selected, Theme, Query, _status.Value, _priority.Value, _list.Viewport.Offset, Draft, _reducedMotion, Width, Height);
        public void Dispose() { _retry.Dispose(); _title.Dispose(); _list.Dispose(); _async.Dispose(); _filtered.Dispose(); _query.Dispose(); _status.Dispose(); _priority.Dispose(); _theme.Dispose(); }
...
internal sealed record IssueBrowserIdentity(int Rows, int Realized, string Selected, string Theme, string Query, string? Status, string? Priority, int Scroll, string Draft, bool ReducedMotion, int Width, int Height);
