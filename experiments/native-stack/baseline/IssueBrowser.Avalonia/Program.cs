...
internal static class Program
{
    ...
    private static int Walkthrough(string? output)
    {
        ...
        var (steps, assigneeFilter) = RunMounted(path, session);
    ...
    private static (List<Step> Steps, AssigneeFilterCheck AssigneeFilter) RunMounted(string path, IssueBrowserSession session)
    {
...
public sealed class IssueBrowserSession : INotifyPropertyChanged
{
    ...
    public ObservableCollection<IssueRow> Rows { get; } = []; private string _query = "", _title = "", _theme = "light", _focus = "search"; private bool _openOnly, _highOnly, _adaOnly, _pending; private string? _error; private int _scrollIndex; private IssueRow? _selected;
    ...
    public SemanticItem[] Semantics => [new("search", "edit", "Search issues", ["set-value"]), new("ada", "checkbox", "Ada", ["toggle"]), new("open", "checkbox", "Open", ["toggle"]), new("high", "checkbox", "High", ["toggle"]), new("issues", "list", "Issues", ["select", "scroll"]), new("title", "edit", "Issue title", ["set-value"]), new("retry", "button", "Retry", ["press"])];
    private void Refresh() { Rows.Clear(); foreach (var row in _all.Where(row => row.Title.Contains(Query, StringComparison.OrdinalIgnoreCase) && (!OpenOnly || row.Status == "Open") && (!HighOnly || row.Priority == "High") && (!AdaOnly || row.Assignee == "Ada"))) Rows.Add(row); if (Selected is not null && !Rows.Contains(Selected)) Select(Rows.FirstOrDefault()); On(nameof(Rows)); On(nameof(RowItems)); On(nameof(MatchSummary)); }
