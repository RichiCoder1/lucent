...
public sealed class IssueBrowserSession : INotifyPropertyChanged
{
    ...
    public ClosedFilterCheck VerifyClosedFilter() { Select(_all[0]); var selected = Selected; var title = Title; var theme = Theme; var scroll = ScrollIndex; var reduced = ReducedMotion; Query = "auth"; OpenOnly = false; ClosedOnly = true; HighOnly = false; AdaOnly = false; Refresh(); return new(ClosedOnly && !OpenOnly && Rows.Count == 667 && Selected == selected && Title == title && Theme == theme && ScrollIndex == scroll && ReducedMotion == reduced, 667, Rows.Count, "Closed"); }
