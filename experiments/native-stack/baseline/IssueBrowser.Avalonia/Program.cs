...
internal static class Program
{
    ...
    private static int Walkthrough(string? output)
    {
        ...
        var steps = RunMounted(path, session);
    ...
    private static List<Step> RunMounted(string path, IssueBrowserSession session)
    {
...
public sealed class IssueBrowserSession : INotifyPropertyChanged
{
    ...
    public ObservableCollection<IssueRow> Rows { get; } = []; private string _query = "", _title = "", _theme = "light", _focus = "search"; private bool _openOnly, _highOnly, _pending; private string? _error; private int _scrollIndex; private IssueRow? _selected;
    public event PropertyChangedEventHandler? PropertyChanged;
    public List<string> FocusTrace { get; } = ["search"]; public bool ReducedMotion { get; private set; }
    public string Query { get => _query; private set => Set(ref _query, value); } public bool OpenOnly { get => _openOnly; private set { if (Set(ref _openOnly, value)) On(nameof(OpenChecked)); } } public bool? OpenChecked => OpenOnly; public bool HighOnly { get => _highOnly; private set { if (Set(ref _highOnly, value)) On(nameof(HighChecked)); } } public bool? HighChecked => HighOnly; public IssueRow? Selected { get => _selected; private set { if (Set(ref _selected, value)) { On(nameof(SelectedItem)); On(nameof(DetailKey)); On(nameof(SelectedTitle)); On(nameof(SelectedStatus)); On(nameof(SelectedPriority)); } } } public object? SelectedItem => Selected; public string Title { get => _title; private set => Set(ref _title, value); } public bool Pending { get => _pending; private set => Set(ref _pending, value); } public string? Error { get => _error; private set => Set(ref _error, value); } public int ScrollIndex { get => _scrollIndex; private set => Set(ref _scrollIndex, value); } public System.Collections.IEnumerable? RowItems => Rows; public string DetailKey => Selected?.Id ?? "No selection"; public string SelectedTitle => Selected?.Title ?? "Select an issue"; public string SelectedStatus => Selected?.Status ?? "Open"; public string SelectedPriority => Selected?.Priority ?? "High"; public string MatchSummary => $"{Rows.Count} matching"; public string Theme { get => _theme; private set { if (Set(ref _theme, value)) On(nameof(RequestedTheme)); } } public ThemeVariant? RequestedTheme => Theme == "dark" ? ThemeVariant.Dark : ThemeVariant.Light; public string Focus { get => _focus; private set => Set(ref _focus, value); } public int RealizedCeiling => 84;
    ...
    public void ToggleOpen() { OpenOnly = !OpenOnly; Refresh(); }
    ...
    public SemanticItem[] Semantics => [new("search", "edit", "Search issues", ["set-value"]), new("open", "checkbox", "Open", ["toggle"]), new("high", "checkbox", "High", ["toggle"]), new("issues", "list", "Issues", ["select", "scroll"]), new("title", "edit", "Issue title", ["set-value"]), new("retry", "button", "Retry", ["press"])];
    private void Refresh() { Rows.Clear(); foreach (var row in _all.Where(row => row.Title.Contains(Query, StringComparison.OrdinalIgnoreCase) && (!OpenOnly || row.Status == "Open") && (!HighOnly || row.Priority == "High"))) Rows.Add(row); if (Selected is not null && !Rows.Contains(Selected)) Select(Rows.FirstOrDefault()); On(nameof(Rows)); On(nameof(RowItems)); On(nameof(MatchSummary)); }
