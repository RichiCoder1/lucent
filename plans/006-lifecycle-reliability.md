...
5. Add one bounded Workbench use case: `IProblemLoader.LoadAsync(workspace,
   ...
   Plan 007 replaces only the placeholder loader implementation.
...
- Custom automation peers beyond the two accessibility-only third-party-control
  ...
  telemetry, or packaging (Plan 007).
...
to placeholder problems; Plan 007 may replace its implementation, not its async
...
Plan 007 consumes these deterministic shutdown/settings/headless gates for
