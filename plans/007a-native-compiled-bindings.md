...
>
> Stop if Avalonia 12.1.1 cannot update a recycled `ListBox` template through
> an inherited `DataContext`, or if coded `CompiledBinding.Create` cannot bind
> a native `AvaloniaProperty` without reflection.
...
A compiled item path can follow the recycled root's new `DataContext`, allowing
safe native recycling without a Lucent component owner.
...
- Outside an item template, the first path segment must identify one stable
  project object: an ordinary component member or component parameter. Pass
...
## Item-template recycling contract

Classify each first-class item template as one of two generated shapes:

### Recyclable native template

Emit `supportsRecycling: true` only when:
...
If any item-dependent value is an ordinary expression, retain the current
`supportsRecycling: false` lowering and null-clear guard. Mixed templates may
still contain explicit native bindings, but they do not claim root reuse.

This distinction is semantic and deterministic. Do not infer that an arbitrary
C# expression is equivalent to a binding path.
...
- For two-way default modes, require a writable leaf. Surface Avalonia
  validation/errors normally; do not mirror them into Lucent state.
- For an ordinary-member source, its object identity is stable for the mounted
...
recyclable-template and explicit-source cases.
...
- Diagnostics distinguish unsupported path shape, non-bindable native target,
  incompatible value type, non-writable two-way leaf, and forbidden Lucent
  reactive source.
...
`TextBox.Text`, null clearing, validation propagation, and a constrained
`ListBox` passing an existing template root during recycling. Record which
expression-tree shapes succeed and the exact target-property API.

**Verify**: a recycled root keeps identity while its displayed item changes;
`INotifyPropertyChanged` updates the visible target; editing a TextBox updates a
writable source through the target's default mode.
...
### 3. Lower recyclable item templates

Classify item templates using the contract above. Emit compiled paths with
inherited DataContext and enable recycling only for fully recyclable templates.
Retain current direct/non-recycling lowering for mixed or ordinary item
expressions. Do not create row owners.
...
**Verify**: headless tests observe reused root identity, correct values at both
ends of a 10,000-item list, bounded realization, notification updates, null
clear, and no retained row owners. A mixed-expression template remains
non-recycling.
...
- [ ] Ordinary Lucent property expressions retain direct generated assignment
      and source-specific invalidation behavior.
- [ ] `binding(item.Path)` lowers to native compiled binding without reflection.
- [ ] Only fully binding-safe native item templates enable root recycling.
- [ ] Recycled roots update identity, displayed values, nested notifications,
      and null clears under the existing headless harness.
- [ ] Explicit-source native bindings honor the target's default mode, replace
      parameter sources, propagate native validation, and dispose exactly once.
- [ ] Unsupported path shapes and non-bindable targets receive source-mapped
      diagnostics rather than silent fallback.
- [ ] Completion, hover, and definition use project/item types through the
      shared semantic model.
- [ ] No Lucent runtime type, notification adapter, row owner, wrapper control,
      registry, reflection path, or new dependency is added.
- [ ] Generated snapshots, SDK consumer, full tests, smokes, docs, and both
      independent reviews pass.

## STOP conditions

- Avalonia does not update compiled bindings when a ListBox changes a recycled
  template root's inherited `DataContext`.
